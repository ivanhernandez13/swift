...
static inline bool task_priority_is_unspecified(JobPriority priority) {
  return priority == JobPriority::Unspecified;
}

static inline bool swift_task_is_unstructured(JobFlags jobFlags) {
  return !jobFlags.task_isAsyncLetTask() && !jobFlags.task_isGroupChildTask();
}

static inline bool swift_task_is_detached(TaskCreateFlags createFlags, JobFlags jobFlags) {
  return swift_task_is_unstructured(jobFlags) && !createFlags.copyTaskLocals();
}
...
static AsyncTaskAndContext swift_task_create_commonImpl(
  ...
  JobPriority basePriority = (taskCreateFlags.getCreationPriority());


  if (swift_task_is_detached(taskCreateFlags, jobFlags)) {
    ...
    SWIFT_TASK_DEBUG_LOG("Creating detached task from current_task %p.", currentTask);
    SWIFT_TASK_DEBUG_LOG("New detached task base pri = %zu", basePriority);

  } else if (swift_task_is_unstructured(jobFlags)) {

    if (task_priority_is_unspecified(basePriority)) {
      // Case 1: No priority specified
      //    Base priority = Base priority of parent with a UI -> IN downgrade
      //    Escalated priority = UN
      if (currentTask) {
        basePriority = currentTask->_private().BasePriority;
        SWIFT_TASK_DEBUG_LOG("Creating unstructured task from current_task %p at %zu", currentTask, basePriority);
      } else {
        basePriority = swift_task_getCurrentThreadPriority();
        SWIFT_TASK_DEBUG_LOG("Creating unstructured task from current thread at %zu", basePriority);
      }
      basePriority = withPriorityDowngrade(basePriority);
      SWIFT_TASK_DEBUG_LOG("New unstructured task base pri = %zu", basePriority);
    } else {
      // Case 2: User specified a priority
      //    Base priority = user specified priority
      //    Escalated priority = UN
      SWIFT_TASK_DEBUG_LOG("New unstructured task with user specified base pri = %zu", basePriority);
    }
    ...
    if (basePriority == JobPriority::Unspecified) {
      // Case 1: No priority specified
      //    Base priority = Base priority of parent with a UI -> IN downgrade
      //    Escalated priority = Escalated priority of parent with a UI -> IN
      //    downgrade
      JobPriority parentBasePri = parent->_private().BasePriority;
      SWIFT_TASK_DEBUG_LOG("Creating structured task from parent %p at %zu", parent, parentBasePri);
      basePriority = withPriorityDowngrade(parentBasePri);
      SWIFT_TASK_DEBUG_LOG("New structured task with propagated base pri = %zu", basePriority);
    } else {
      // Case 2: User priority specified
      //    Base priority = User specified priority
      //    Escalated priority = Escalated priority of parent with a UI -> IN
      //    downgrade
      SWIFT_TASK_DEBUG_LOG("Creating structured task from parent %p", parent);
      SWIFT_TASK_DEBUG_LOG("New structured task with user specified base pri = %zu", basePriority);
    }
  ...
  // TODO (rokhinip): This shouldn't be needed, why is this needed to make
  // things pass and to kick shit out of the main run loop ??
  if (basePriority == JobPriority::Unspecified) {
  ...
  bool captureCurrentVoucher = !swift_task_is_detached(taskCreateFlags, jobFlags);
  if (asyncLet) {
